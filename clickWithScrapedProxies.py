...
async def scrape_proxies(method, num_proxies):
    proxies = set()

    # Scraping proxies from spys.me
    url = f"https://spys.me/{method}.txt"
    response = await httpx.get(url)
    pattern = r"\d{1,3}(?:\.\d{1,3}){3}(?::\d{1,5})?"
    proxy_list = re.findall(pattern, response.text)

    proxies.update(proxy_list)

    # Add more scraping logic for other proxy sources if desired

    return random.sample(proxies, num_proxies)


async def increase_link_click(url, num_clicks):
    proxies = await scrape_proxies("http", num_clicks)
...
click_count = 100  # Replace with the desired number of clicks

loop = asyncio.get_event_loop()
loop.run_until_complete(increase_link_click(link, click_count))
loop.close()
